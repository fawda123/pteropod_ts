...
tomod <- chmsub %>% 
  ...
  select(ara, chla, oxy, sal, temp, typ3, depth) %>% 
  na.omit()

totab <- combn(names(tomod), 2) %>% 
  t %>% 
  as_tibble %>% 
  ...
  mutate(
    ...
    corv = stringi::stri_pad_right(corv, width = 4, pad = '0')
  ) %>% 
  ...
  mutate(
    var1 = case_when(
      var1 == 'ara' ~ 'Aragonite saturation ($\\Omega$)', 
      var1 == 'chla' ~ 'Chlorophyll-a ($\\mu$g/L)', 
      var1 == 'oxy' ~ 'Dissolved Oxygen (mg/L)', 
      var1 == 'sal' ~ 'Salinity (psu)', 
      var1 == 'temp' ~ 'Temperature (C)', 
      var1 == 'typ3' ~ 'Dissolution (%)' 
    )
  )
