...
column(12, 
  ...
  column(4, 
    selectInput('dis', 'Select dissolution:', choices = c('typ1', 'typ2', 'typ3', 'typ23'))
  )
)
```
...
renderPlot({
  ...
  dat <- chmdat %>% 
    filter(var %in% chm) %>% 
    full_join(biodat, by = c('station', 'date', 'yr', 'mo')) %>% 
    select(-lon, -lat, -var, -abu, -avesz, -stdsz) %>% 
    gather('dissvar', 'dissval', typ1:typ23) %>% 
...
renderPlot({
  ...
  # bio
  toplo1 <- biodat %>% 
    select(date, yr, mo, station, typ1, typ2, typ3, typ23) %>% 
    gather('diss', 'val', typ1:typ23) %>% 
    filter(diss %in% dis)
