...
The second set of plots show the selected environmental measure against temperature with points sized by the pteropod response measure. Plots are facetted by cohort-year with points in each facet colored by month. 
```{r}
renderPlot({
  ...
  p <- ggplot(dat, aes_string(x = 'temp', y = chm, size = 'dissval')) +
    ...
    ylab(paste(val, chm)) +
    xlab(paste(val, 'temp')) +
    scale_size(paste(val, dis), range = c(2, 10)) +
    theme_bw(base_family = 'serif', base_size = 16) +
    theme(
      strip.background = element_blank(), 
      # legend.title = element_blank(), 
