...
lmmodtmp <- reactive({
  ...
  frm <- paste('dissval ~', input$chm, '+temp')
  if(modtyptmp)
    frm <- gsub('\\+', '*', frm)
...
renderPlot({
  ...
  p <- ggplot(tmpdat, aes_string(x = 'temp', y = chm, size = 'dissval')) +
...
renderPrint({
  ...
  validate(
    need(input$chm != 'temp', 'model not possible with selection')
  )
  
  # input
  lmmodtmp <- lmmodtmp()$July
  
  vifval <- lmmodtmp$model[,-1] %>% 
    lm(temp ~ ., data= .) %>% 
    summary %>% 
    .$r.squared
  vifval <- 1 / (1 - vifval)
...
renderPlot({
  ...
  lmmodtmpplo <- lmmodtmp$July
  
  # colors
  cols <- RColorBrewer::brewer.pal(9, 'RdBu')
  
  # get data to plot
  pldat <- get_pldat(lmmodtmpplo, 'temp')
  pl1 <- pldat[[1]]
  pl2 <- pldat[[2]]
  ...
  p1 <- ggplot() +
    ...
    scale_colour_gradientn(paste(tmp, 'temp'), colours = cols) + 
...
renderPlot({
  ...
  lmmodtmpplo <- lmmodtmp$September
...
column(12, 
column(width = 4, 
       selectInput('modtypoxy', "Include interaction:", choices = c(T, F))
       )
)
```
...
renderPrint({
  ...
  # input
  lmmodoxy <- lmmodoxy()$July
  
  vifval <- lmmodoxy$model[,-1] %>% 
    lm(oxy ~ ., data= .) %>% 
    summary %>% 
    .$r.squared
  vifval <- 1 / (1 - vifval)
...
renderPlot({
  ...
  lmmodoxyplo <- lmmodoxy$July
  
  # colors
  cols <- RColorBrewer::brewer.pal(9, 'RdBu')
  
  # get data to plot
  pldat <- get_pldat(lmmodoxyplo, 'oxy')
  pl1 <- pldat[[1]]
  pl2 <- pldat[[2]]
...
renderPlot({
  ...
  lmmodoxyplo <- lmmodoxy$September
...
The second set of plots show the selected environmental measure against temperature with points sized by the pteropod response measure. The x-axis is alway temperature and can be selected to show minimum, average, and maximum. Plots are facetted by month with points in each facet colored by cohort year. 
```{r}
column(12, 
  column(4, 
...
The third set of plots show the selected environmental measure against oxygen with points sized by the pteropod response measure. The x-axis is alway oxygen and can be selected to show minimum, average, and maximum. Plots are facetted by month with points in each facet colored by cohort year. 
```{r}
column(12, 
  column(4, 
...
grodat <- reactive({
  ...
  dat <- chmdatsum %>% 
    filter(var %in% chm) %>% 
    full_join(biodat, by = c('station', 'date', 'yr', 'cohortyr', 'mo')) %>% 
    select(date, yr, cohortyr, mo, station, len, !!dis, ave:dlt) %>% 
    gather('chemvar', 'chemval', ave:dlt) %>% 
    filter(chemvar %in% val) %>% 
    filter(station %in% sta) %>% 
    rename(disval = !!dis)
...
gromod <- reactive({
  ...
  # complete model
  allchm <- lm(len ~ chemval, data = grodat)
  alldis <- lm(len ~ disval, data = grodat)
  ...
  bycohortyr <- grodat %>% 
    ...
    nest %>% 
    mutate(
      modchm = purrr::map(data, function(x){ try({lm(len ~ chemval, data = x)})}),
      moddis = purrr::map(data, function(x){ try({lm(len ~ disval, data = x)})})
    )
  bycohortyroutchm <- pull(bycohortyr, modchm)
  names(bycohortyroutchm) <- pull(bycohortyr, cohortyr) %>% paste0(., 'chm')
  bycohortyroutdis <- pull(bycohortyr, moddis)
  names(bycohortyroutdis) <- pull(bycohortyr, cohortyr) %>% paste0(., 'dis')
  ...
  byseasoutchm <- pull(byseas, modchm)
  names(byseasoutchm) <- pull(byseas, mo) %>% paste0(., 'chm')
  byseasoutdis <- pull(byseas, moddis)
  names(byseasoutdis) <- pull(byseas, mo) %>% paste0(., 'dis')
...
renderPlot({
  ...
  p <- ggplot(grodat, aes(x = chemval, y = len)) +
    # geom_line(aes(group = station), colour = 'grey') +
    stat_smooth(method = 'lm', se =T, colour = 'red') +
...
renderPrint({lapply(gromod()['allchm'], summary)})
```
...
renderPrint({lapply(gromod()[c('2014chm', '2015chm', '2016chm')], summary)})
```
...
renderPrint({lapply(gromod()[c('Julchm', 'Sepchm', 'Aprchm')], summary)})
```
...
renderPlot({
  ...
  p <- ggplot(grodat, aes(x = disval, y = len)) +
    # geom_line(aes(group = station), colour = 'grey') +
    stat_smooth(method = 'lm', se =T, colour = 'red') +
    geom_label(aes(fill = mo, group = station, label = station), size = 3, colour = 'grey') +
    ylab("Average size") +
...
renderPrint({lapply(gromod()['alldis'], summary)})
```
...
renderPrint({lapply(gromod()[c('2014dis', '2015dis', '2016dis')], summary)})
```
...
renderPrint({lapply(gromod()[c('Juldis', 'Sepdis', 'Aprdis')], summary)})
```
