...
column(12, 
  column(4, 
    selectInput('oxy', 'Select x-axis oxygen value:', choices = c('min', 'ave', 'max', 'std'))
  )
)
...
lmmod <- reactive({
  ...
  modapr <- lm(frm, data = datapr) %>% 
    step(trace = 0) 
  
  modsep <- lm(frm, data = datsep) %>% 
    step(trace = 0) 
  
  return(list(September = modsep, April = modapr))
...
renderPlot({
  ...
  chkfrm <- lmmodplo$call$formula %>% 
    as.character %>% 
    paste(collapse = ' ')
  validate(
    need(grepl('\\:|\\*', chkfrm), 'No interaction in September model')
  )
...
renderPlot({
  ...
  validate(
    need(grepl('\\:|\\*', chkfrm), 'No interaction in April model')
  )
...
column(12, 
  column(4, 
    selectInput('oxy2', 'Select x-axis temperature value:', choices = c('min', 'ave', 'max', 'std'))
  )
)
