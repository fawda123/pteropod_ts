...
Combined model, average aragonite (above is minimum).
```{r eval = T}
#data
tomod <- chmdatsum %>% 
  filter(var %in% 'ara') %>% 
...
toprd <- data.frame(
    chemval = c(ave + 2 * 0.5, ave, ave - 2 * 0.5), 
    lab = c('pre', 'current', 'post')
  ) %>% 
  mutate(
