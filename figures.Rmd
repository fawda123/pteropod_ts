...
bsmap <-  get_map(location = dat_ext, maptype = 'terrain-background', zoom = 8)
...
```{r pcafig, fig.cap = "Results of prinicipal components analysis for environmental variables collected at each site for each sample date.  Environmental variables included temperature, salinity, dissolved oxygen, and aragonite saturation state. The left plot shows site groupings based on dominant clusters shown in Figure \\@ref(fig:clstmap) with site points sized by measured type III dissolution for pteropods collected at the same time.  The right plot shows the sites with text identifies for the sit number, followed by the month and sample year.", out.width = "90%", fig.align = 'center'}
...
jpeg('figs/obsdat.jpg', height = 7, width = 7, units = 'in', res = 300, family = 'serif')
wrap_elements(pleg) + 
...
```{r obsdat, fig.cap = "Observed data", out.width = "80%", fig.align = 'center'}
