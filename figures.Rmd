...
p1 <- ggplot(toplo, aes(x = chemval, y = dissval)) +
  ...
  scale_fill_manual('Within months, across years', values = colmos) + 
  scale_colour_manual('Within months, across years', values = colmos) + 
...
p2 <- ggplot(toplo, aes(x = chemval, y = dissval)) +
  ...
  scale_fill_manual('Within years, across months', values = colmos) + 
  scale_colour_manual('Within years, across months', values = colmos) + 
  guides(colour = guide_legend(override.aes = list(size = 4, alpha = 0.6)))
...
```{r disvara, fig.cap = "Percent type III dissolution measured in pteropods versus minimum observed aragonite saturation state for each station.  The top row (a-c) shows stations grouped by month across cohort years and the bottom row (d-f) shows stations grouped by cohort years across months.  Linear regression lines with 95\\% confidence intervals in dark grey and 95\\% prediction intervals in light grey are shown in each panel.", out.width = "80%", fig.align = 'center'}
