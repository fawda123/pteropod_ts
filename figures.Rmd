...
plo1dat <- oxydat %>% 
  filter(var %in% 'ara' & chemvar %in% 'min' | var %in% 'oxy' & chemvar %in% 'min') %>% 
  select(-chemvar) %>% 
  spread(var, chemval) %>% 
  filter(mo %in% c('Jul')) 
...
p1 <- ggplot() +
  ...
  coord_cartesian(ylim = c(-10, 53), xlim = c(0.19, 0.8)) +
  scale_colour_gradientn(expression(paste('Minimum\nobserved ',O[2])), colours = cols) 
...
p2 <- ggplot(plo2dat, aes(x = oxy, y = ara, size = dissval)) + 
  geom_point(aes(fill = mo), pch = 21, alpha = 0.8) + 
  scale_fill_manual(values = colmos) + 
  theme_bw() +
  # geom_smooth(method = 'lm', se = F, colour = 'black') +
  scale_size('% type III\ndissolution', range = c(2, 9)) + 
  scale_y_continuous(expression("Minimum observed  "*Omega)) +
  scale_x_continuous(expression(paste('Minimum observed ',O[2]))) +
  guides(fill = guide_legend(title = element_blank(), override.aes = list(size = 4)))
jpeg('figs/oxyint.jpg', family = 'serif', height = 7, width = 5, res = 300, units = 'in')
p2 + p1 + plot_layout(ncol = 1)
dev.off()
```

```{r oxyint, fig.cap = 'Relationships of dissolved oxygen and aragonite saturation state with dissolution measures of pteropods.  The top plot shows the minimum observed aragonite saturation state at a site against minimum observed disssolved oxygen, with points sized by average dissolution and colored by month.  The bottom plot shows results for a linear model of dissolution against aragonite state and oxygen that includes both main and interaction effects (July only).  The color range depicts the minimum and maximum observed values for average dissolved oxygen across all stations.', fig.align = 'center', out.width = "55%"}
...
dev.off()
```
```{r groplo, fig.cap = 'Growth cohorts represented as summer and fall individauls with measured lengths against time.  Growth trends within each cohort are shown with linear regression lines.', fig.align = 'center', out.width = "90%"}
